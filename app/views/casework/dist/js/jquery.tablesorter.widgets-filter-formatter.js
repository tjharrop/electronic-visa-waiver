...
tsff = ts.filterFormatter = {
	...
	uiDatepicker: function($cell, indx, defDate) {
		...
		$hdr = $cell.closest('thead').find('th[data-column=' + indx + ']').addClass('filter-parsed');
		// Add date range picker
		t = '<label style="display:none">' + o.textFrom + '</label><input type="text" class="dateFrom" style="border: solid 1px #dee0e2;width:90px;float:left" placeholder="' +
			($hdr.data('placeholderFrom') || $hdr.attr('data-placeholder-from') || c.widgetOptions.filter_placeholder.from || '') + '" />' +
			'<label style="display:none">' + o.textTo + '</label><input type="text" class="dateTo" style="border: solid 1px #dee0e2;width:90px" placeholder="' +
